...
module.exports = {
        ...
        run: async (client, message, args, config, language) => {
            ...
            const ticketchannel = message.guild.channels.find(channel => channel.name === `${config.docticket}`);
            const ticketopen = message.guild.channels.find(channel => channel.name === `${congig.opentickets}`);
